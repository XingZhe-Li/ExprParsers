...
对于LALR的补充说明: 这个合并是基于同心集的，换句话说就是Item闭包后去掉LookAhead，如果两个这样的闭包除去Lookahead都是相同的就合并，而不是以Item为单位进行相同判定来合并。事实上，如果以Item为单位进行合并且没有手动干预（即所有的Lookahead都是从生成式推导出来的），其最终得到的结果和SLR是一样的。
